...
cronedit () {
  ...
  echo "$backup_cron_freq flock -n /tmp/backup.lock /home/scripts/backup_script.sh >> /config/logs/backup_script-\$(date +%F).log && cp /config/logs/backup_script-\$(date +%F).log /mnt/sftp/statistik/backup_script-\$(date +%F).log && umount -lf /mnt/sftp/">>cron.temp
  crontab cron.temp
  rm -f cron.temp
