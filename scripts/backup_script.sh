...
incremental_backup() {
    get_last_backup
    rsync_flags="-avq --no-perms --delete --timeout=30 --stats --log-file $logs_folder"rsync-"$backup_start_date".log" --bwlimit $backup_bwlimit --link-dest=$dest_folder$last_backup/ $sftp_backup_folder $dest_folder$backup_start_date"
    run_rsync
}
...
initial_backup() {
    rsync_flags="-avq --no-perms --delete --timeout=30 --stats --log-file $logs_folder"rsync-"$backup_start_date".log" --bwlimit $backup_bwlimit $sftp_backup_folder $dest_folder$backup_start_date"
    run_rsync
}
